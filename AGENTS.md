...
The pipeline is one-way; on-disk and API contracts are versioned explicitly (`v: 1` for the API/heartbeat shapes, `v: 2` for the prompt cache).
...
- **Versioned serializable contracts**: the persisted cache uses `v: 2` because its prompt entries now include assistant replies; heartbeat and API snapshot shapes remain `v: 1`.
